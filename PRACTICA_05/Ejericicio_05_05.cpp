...
// Problema planteado: Leer 2 vectores de dimensión N y combinarlos en otro vector.

#include <iostream>
using namespace std;

void LeerVectores(int vec1[], int vec2[], int N);
void CombinarVectores(int vec1[], int vec2[], int resultado[], int N);
...
int main() 
{
    ...
    CombinarVectores(vec1, vec2, resultado, N);
...
void DesplegarVector(int vector[], int N) 
{
    cout << "Resultado de la combinación de los vectores:\n";
    for (int i = 0; i < N; i++) 
    {
        cout << vector[i] << " ";
    }
